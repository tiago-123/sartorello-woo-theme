...
<?php else : ?>
	...
	<h2 class="mb-8 pb-4 text-xl font-semibold border-b border-neutral-200">
		<?php
		printf(
			!empty($address) ? esc_html__('Edit %s', 'woocommerce') : esc_html__('Add %s', 'woocommerce'),
	...
	<form method="post" novalidate>
		...
		<div class="woocommerce-address-fields">
			...
			<div
				x-init="window.<?php echo esc_js($load_address); ?>Fields = $data"
				x-data="fieldValidation(<?php echo esc_js(json_encode(array_keys($address))); ?>)"
				...
				foreach ($address as $key => $field) {
					...
					$input_value = wc_get_post_data_by_key($key, $field['value']) ?? '';
			...
			<p>
				<button type="submit" class="mt-6 px-4 py-3 text-sm font-medium text-white rounded-sm bg-neutral-900 hover:bg-neutral-700" name="save_address" value="<?php esc_attr_e('Save address', 'woocommerce'); ?>"><?php esc_html_e('Save address', 'woocommerce'); ?></button>
